...
class TopAwaitRemoteMediator(
    ...
    override suspend fun load(
        loadType: LoadType,
        state: PagingState<Int, TopAwaitDb>
    ): MediatorResult {
        return try {
            ...
            val response = api.getTop(KinopoiskTopType.TOP_AWAIT_FILMS.name, page)

            dao.saveFilmList(response.films.map { it.toFilmEntity() })
            dao.saveTopAwaitList(response.films.map { it.toTopAwaitEntity() })
